...
A single real dataset, a non-canonical split with no argued
...
- **Both propagators beat raw features by a wide margin on every
  ...
  finding of this whole test is that non-backtracking-walk structure
...
- **GCN's propagator wins on every dataset, at both the linear and deep-
  ...
  this  test  for a genuine (if disappointing, relative to the original
